...
namespace vlr {
    ...
    void MPCController::currentstateHandler(const path_follower::state_Dynamic current_state) {
        ...
        //double factor = min(state_(3)/8. - 0.3, 1.);
        //p_Q(2, 2) = p_q_theta * max(factor, 0.);
        //p_Q(5, 5) = p_q_theta_dot * max(factor, 0.);
        //p_R(1, 1) = p_r_delta * max(8./(state_(3)+0.1), 1.);
        //p_R_delta(1, 1) = p_rd_delta * max(8./(state_(3)+0.1), 1.);
    }
...
end_time_mpc = Time::current();
    ...
    void MPCController::kinematics( const Eigen::Matrix<double, NUM_STATES, 1> &s, 
                                  const Eigen::Matrix<double, NUM_CONTROLS, 1> &u_, 
                                  Eigen::Matrix<double, NUM_STATES, 1> *s_dot, 
                                  Eigen::Matrix<double, NUM_STATES, NUM_STATES> *A, 
                                  Eigen::Matrix<double, NUM_STATES, NUM_CONTROLS> *B) {
        ...
        double th_dot = s(5), u_dot = u_(0);
        double del = u_(1), tan_del = tan(u_(1)), cos_del = cos(u_(1));
        double Beta, a = p_vs.param.a, b = p_vs.param.b, I = p_vs.param.iz;
        Beta = atan2(b * tan_del , a + b);

        (*s_dot)(0) = u / cos(Beta) * cos(s(2) + Beta);
        (*s_dot)(1) = u / cos(Beta) * sin(s(2) + Beta);
        (*s_dot)(2) = u * tan(Beta) / b;
        (*s_dot)(3) = u_dot;
        (*s_dot)(4) = 0;
        (*s_dot)(5) = (u * tan(Beta) / b - s(5)) / 0.05;
        //(*s_dot)(4) = tan_del * (u_dot - th_dot * v) + (Fyf / cos_del + Fyr) / m - th_dot * u;
        //(*s_dot)(5) = m * a / I * tan_del * (u_dot - th_dot * v) + a * Fyf / (I * cos_del) - b * Fyr / I;
        ...
        if (A != 0) {
            ...
            for (int i = 0; i < NUM_STATES; i++) {
                s2(i) += EPSILON;
                kinematics(s2, u_, &s_dot1, 0, 0);
                s2(i) -= 2 * EPSILON;
                kinematics(s2, u_, &s_dot2, 0, 0);
                s2(i) += EPSILON;
                A->col(i) = (s_dot1 - s_dot2) / (2 * EPSILON);
            }
        }
        ...
        if (B != 0) {
            ...
            for (int i = 0; i < NUM_CONTROLS; i++) {
                u2(i) += EPSILON;
                kinematics(s, u2, &s_dot1, 0, 0);
                u2(i) -= 2 * EPSILON;
                kinematics(s, u2, &s_dot2, 0, 0);
                u2(i) += EPSILON;
                B->col(i) = (s_dot1 - s_dot2) / (2 * EPSILON);
            }
        }
	...
	kinematics(s, u, &k1);
        kinematics(s + 0.5 * dt * k1, u, &k2);
        kinematics(s + 0.5 * dt * k2, u, &k3);
        kinematics(s + dt * k3, u, &k4);
        (*s_next) = s + dt * (k1 / 6.0 + k2 / 3.0 + k3 / 3.0 + k4 / 6.0);
	}
