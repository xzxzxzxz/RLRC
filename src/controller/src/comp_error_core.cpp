...
double getDistance(const path_follower::TrajectoryPoint2D point, const path_follower::state_Dynamic state, double ra_to_cg)
{
  ...
  double x_cg = state.X + cos(state.psi) * ra_to_cg;
  double y_cg = state.Y + sin(state.psi) * ra_to_cg;
  double dist = sqrt(pow(x_traj_cg - x_cg, 2) + pow(y_traj_cg - y_cg, 2));
  return dist;
}
...
vector<double> ComputeTrackingError(const path_follower::Trajectory2D traj, const path_follower::state_Dynamic state, double ra_to_cg)
{
  ...
  {
    if (getDistance(traj.point[index], state, ra_to_cg) > getDistance(traj.point[i], state, ra_to_cg))
      index = i; 
  }
  ...
  double delta_x = x_traj_cg - x_cg;
  double delta_y = y_traj_cg - y_cg;
