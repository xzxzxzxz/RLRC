...
void DOB_ros_step(void)
{
  ...
  rtb_D1 = DOB_ros_P.Dn[0] * D1_tmp;
  rtb_D1 += DOB_ros_P.Dn[1] * DOB_ros_DW.D1_states[0];
  denIdx = 2;
  for (j = 0; j < 4; j++) {
    rtb_D1 += DOB_ros_P.Dn[denIdx] * DOB_ros_DW.D1_states[j + 1];
    denIdx++;
  }
   ...
   *  Delay: '<Root>/Delay1'
   */
  rtb_Sum4 = DOB_ros_DW.Delay1_DSTATE[0] - rtb_D1;
